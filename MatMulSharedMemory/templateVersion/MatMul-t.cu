...
template<class T>
__global__ void MatrixMulKernel(const T* d_A, const T* d_B, T* d_C, const int rowsA, const int columnsA, const int rowsB, const int columnsB) {
//
// Assistance from
// https://github.com/debowin/cuda-tiled-matrix-multiplication
// is greatly appreciated.
//
// Calculating A[i][k] * B[k][j] = C[i][j]
//
   // Statically sized arrays of GPU shared memory
   __shared__ T tileAs[TILE_WIDTH][TILE_WIDTH];
   __shared__ T tileBs[TILE_WIDTH][TILE_WIDTH];
...
template<class T>
void computeGold(T* C, const T* A, const T* B, const int hA, const int wA, const int wB)
{
  for(int i = 0; i < hA; ++i){
    for(int j = 0; j < wB; ++j){
      T sum = 0.;
...
template<class T>
bool CompareMatrices(T* A, T* B) {
    int size = sizeof(A);

    for (int i = 0; i < size; i++)
        if (abs((float)A[i] - (float)B[i]) > 0.0001f){
...
template<class T>
void writeMatrixFile(const T* A, const int rA, const int cA, char* name){ 
...
// Options for different size and shaped matrices
  ...
  cout << endl << "General Matrix Shared Memory Multiply on GPU: A[i][k] B[k][j] = C[i][j]" << endl; 
  ...
  //
  // Run code using integers
  //
#ifdef VTYPE_INT
    cout << "Testing INTEGER matrix values!!" << endl;
    int size_A = N_A * sizeof(int);
    int size_B = N_B * sizeof(int);
    int size_C = N_C * sizeof(int);

    // Define host and device matrices
    int *A, *B, *C, *gC; // host copies of A, B, C, gold-C
    ...
    gC = (int *)malloc(size_C); 
    fill(gC + 0, gC + N_C, 0);
#endif  
#ifdef VTYPE_FLOAT
    cout << "Testing FLOAT matrix values!!" << endl;
    ...
    cout << "Testing DOUBLE matrix values!!" << endl;
    int size_A = N_A * sizeof(double);
    int size_B = N_B * sizeof(double);
    int size_C = N_C * sizeof(double);

    // Define host and device matrices
    double *A, *B, *C, *gC; // host copies of A, B, C, gold-C
